...
const unsigned int WPM_THRESHOULD = 10;
...
int main(int argc, const char *argv[]) {
    ...
    CFMachPortRef eventTap = CGEventTapCreate(kCGSessionEventTap, kCGHeadInsertEventTap, 0, eventMask, CGEventCallback, &userInfo);
...
CGEventRef CGEventCallback(CGEventTapProxy proxy, CGEventType type, CGEventRef event, void *refcon) {
    if (type != kCGEventKeyDown && type != kCGEventFlagsChanged && type != kCGEventMouseMoved) { 
        return event; 
    }

    // Get the current time.
    time_t currentTime = time(NULL);
    UserInfo *data = (UserInfo *) refcon;
    time_t elapsedTime = currentTime - data->startTime;
